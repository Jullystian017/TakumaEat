    ...
    <section id="about" className="relative overflow-hidden border-t border-black/5 bg-white py-16 sm:py-24">
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-12 px-4 sm:gap-16 sm:px-6">
        ...
        <div className="hide-scrollbar flex gap-6 overflow-x-auto overflow-y-hidden px-5 pb-6 pr-14 snap-x snap-mandatory [scrollbar-width:none] sm:mx-0 sm:flex-none sm:overflow-visible sm:px-0 sm:pb-0 sm:gap-8 sm:pr-0 sm:grid sm:grid-cols-2 xl:grid-cols-3 sm:snap-none">
          ...
          ) : foods.filter(f => f.status !== 'discontinued').length === 0 ? (
            <div className="col-span-full py-10 text-center text-slate-400">Belum ada menu populer saat ini.</div>
          ) : foods.filter(f => f.status !== 'discontinued').map((food, index) => (
            <motion.div
              key={food.id}
              ...
              whileHover={{ y: -12 }}
              className={cn(
                "group flex h-full flex-[0_0_78vw] flex-col overflow-hidden rounded-[28px] border border-black/5 bg-white text-left shadow-[0_20px_55px_rgba(15,23,42,0.08)] transition-all duration-400 sm:flex-none sm:w-full sm:max-w-none md:max-w-[360px]",
                (food.stock > 0 && food.status === 'available') ? "hover:-translate-y-3 hover:border-brand-gold/50 hover:shadow-[0_24px_65px_rgba(15,23,42,0.12)]" : "opacity-75 cursor-not-allowed"
              )}
            >
              <div className="relative h-52 w-full overflow-hidden sm:h-64">
                <Image
                  ...
                  alt={food.name}
                  fill
                  className={cn("object-cover transition-transform duration-700 group-hover:scale-110", (food.stock === 0 || food.status === 'out_of_stock') && "grayscale opacity-50")}
                />
                ...
                <div className="absolute bottom-4 left-5 flex items-center gap-3 text-xs font-medium uppercase tracking-[0.28em] text-white">
                  <span className="inline-flex items-center rounded-full bg-white/20 px-3 py-1">
                    {(food.stock === 0 || food.status === 'out_of_stock') ? 'Out of Stock' : 'Popular'}
                  </span>
                </div>
              </div>
              <div className="flex flex-1 flex-col gap-5 p-6 text-black">
                ...
                <div className="mt-auto flex items-center justify-between">
                  ...
                  <Button
                    type="button"
                    disabled={food.stock === 0 || food.status === 'out_of_stock'}
                    onClick={() =>
                      addItem({
                        name: food.name,
                        price: food.price,
                    ...
                    className={cn(
                      ...
                      (food.stock === 0 || food.status === 'out_of_stock')
                  ...
                  >
                    ...
                    {(food.stock === 0 || food.status === 'out_of_stock') ? 'Habis' : 'Add to Cart'}
                  </Button>
                </div>
              </div>
            </motion.div>
